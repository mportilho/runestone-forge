...
A constant subexpression that fails while being folded is left unfolded, so that it fails during execution exactly as the oracle would, with the same diagnostic code and the same source span. Folding never turns an ordinary language failure into a new compilation diagnostic and never produces a poisoned constant value. Resource-shape violations that semantic resolution can prove before folding are different: ADR 0024 classifies them as terminal compilation diagnostics so dangerous allocation is never attempted. A deferred check is discharged only when its subtree folds successfully; a failed ordinary check abandons the fold and preserves both the node and the check.
...
Operational work exhaustion is intentionally not stable at the exact configured boundary across optimizer or algorithm changes. Folding pays its work during compilation and a folded constant does not pay fictitious runtime work; other specializations may likewise reduce the tier's debit. Codes and general budget dimensions remain stable, but callers must configure operational margin rather than treating the minimum observed debit as language semantics.
