...
- Invocação de funções sem reflexão: entrada sem array para aridades 0–10, tentando `LambdaMetafactory` e usando `invokeExact` com assinatura fixa quando o handle não puder ser ligado; aridades 11+ usam `invokeExact` com spreader pré-adaptado. Os pontos de entrada são gerados **no build do ambiente** e compartilhados por todos os planos daquele ambiente (ADR 0020, com ambiente longevo como pré-condição). **Pré-alocação de arrays de varargs sai**: varargs são rejeitados na importação e descartados no catálogo de tipos Java, logo não há alvo.
