...
**Entregas**
- Famílias especializadas operador × tipo **adicionadas** ao lado das genéricas (`CompareStringEQ`, `NullCoalesceN`, `Between` especializado, concatenação N-ária, etc.), guiadas por `resolvedTypes`, fatos numéricos e nulidade, e escolhidas na construção do plano. A família genérica **permanece** como o nó que `buildOracle` constrói: substituí-la apagaria o substrato do Oráculo que o ADR 0019 exige manter. Cada família entra por ganho pareado próprio; a aritmética decimal binária entra com expectativa baixa registrada e sai por medição se não pagar.
...
- Pool opcional de `ExecutionScope` — **implementar somente se cumprir a condição numérica declarada de antemão** no plano detalhado (§13).
