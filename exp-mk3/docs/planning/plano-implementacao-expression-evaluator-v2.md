...
**Entregas**
- Tipos: `ScalarType` (`NUMBER`, `BOOLEAN`, `STRING`, `DATE`, `TIME`, `DATETIME`), `VectorType`/`CollectionType` (com tipo de elemento), `MapType`, `ObjectType`, `UnknownType`, `NullType`. Decisão implementada da §6: `OffsetDateTime` normalizado para `LocalDateTime` no fuso do ambiente **em compilação** (o offset está no literal), mantendo um único `DATETIME`.
- `ExpressionEnvironment` + builder: `MathContext`, `transcendentalMathContext`, `NumericMode` (`DECIMAL` default / `FAST`), modo estrito, `maxFilterDepth`, `maxVectorSize`, `ExpressionEnvironmentId` (hash determinístico de todo o conteúdo relevante — insumo do cache da Etapa 9).
