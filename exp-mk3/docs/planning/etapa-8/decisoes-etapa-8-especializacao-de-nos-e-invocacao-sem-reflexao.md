...
## Simetria de `MathContext` (decisao revista; ver ADR 0021)

- Analise original encontrou duas assimetrias — `sum` acumula com `add` sem `MathContext` enquanto `avg` divide com ele, e `modulo` usa `remainder` sem `MathContext` — e propos fechar as duas **adicionando** `MathContext` a `sum` e a `modulo`. Revisao: essa direcao estava errada nos dois pontos.
- `+`/`-` tem escala `max(escala1, escala2)`, limitada por operando e nao composta sob repeticao; aplicar `MathContext` ali so descarta digitos que a linguagem aceitou como exatos, sem ganho de desempenho que justifique a perda. `*` tem escala `escala1 + escala2`, que soma a cada repeticao (`reduce` com `*`, laco de taxa composta) — esse caso, sim, exige `MathContext` sempre, incluindo em multiplicacoes isoladas, porque uma regra dependente de escala do operando nao e um contrato enunciavel.
- `modulo` usando `remainder(divisor, mathContext)` nao calcula "o resto exato arredondado" — deriva de um quociente inteiro ja arredondado pelo contexto e pode lancar `ArithmeticException` onde `remainder(divisor)` sem contexto teria sucesso. Isso nao e uma questao de custo: e uma mudanca de semantica incorreta. `modulo` permanece exato.
- Decisao revista: `MathContext` se aplica apenas onde a operacao e obrigada a arredondar (`/`, `^`, `root`, `reciprocal`, transcendentais) ou onde a escala cresce sob repeticao (`*`, sempre). `+`, `-`, `negate`, `abs` e `modulo` permanecem exatos. `sum` permanece acumulacao exata (decorre de `+` exato); `avg` continua somando exato e arredondando uma unica vez na divisao final — a assimetria original se dissolve em vez de ser fechada por adicao de contexto.
- Excecao ja decidida e mantida: **nao** aplicar a `negate`, pelo mesmo motivo original (trocar o sinal e exato; arredondar truncaria um literal aceito como exato).
- Achado adicional durante a revisao: `FinancialBuiltInFunctions.npv` multiplica a taxa por periodo em um laco (`currentRate.multiply(ratePlusOne)`) sem `MathContext` — e exatamente o caso de crescimento sob repeticao que a regra de `*` cobre. Corrigido junto, na mesma issue preparatoria.
- Sequenciamento: **issue preparatoria antes** da Etapa 8, com registro de decisao proprio (agora ADR 0021). Isso muda valores (em `+`/`-` e em `npv`), e a Etapa 8 e uma etapa cuja regra e nao mudar valor; especializar sobre a semantica final evita refazer prova.
- Condicao de custo: nao se aplica na direcao revista. Remover uma chamada de `MathContext` de `+`/`-` nao pode ficar mais lento; nao ha cenario em que a assimetria precise ficar registrada por custo.
