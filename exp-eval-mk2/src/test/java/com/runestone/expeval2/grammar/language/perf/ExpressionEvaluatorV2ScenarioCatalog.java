...
public final class ExpressionEvaluatorV2ScenarioCatalog {

    private static final String CORPUS_INDEX_RESOURCE = "com/runestone/expeval2/grammar/language/perf/corpus/index.tsv";
    ...
    private ExpressionEvaluatorV2ScenarioCatalog() {
    }
    ...
    private static List<Scenario> loadCorpusScenarios() {
        try (BufferedReader reader = openResource(CORPUS_INDEX_RESOURCE)) {
            return reader.lines()
                ...
                .map(ExpressionEvaluatorV2ScenarioCatalog::parseCorpusRow)
                .toList();
        } catch (IOException e) {
    ...
    private static BufferedReader openResource(String resourcePath) throws IOException {
        InputStream inputStream = ExpressionEvaluatorV2ScenarioCatalog.class.getClassLoader().getResourceAsStream(resourcePath);
        if (inputStream == null) {
