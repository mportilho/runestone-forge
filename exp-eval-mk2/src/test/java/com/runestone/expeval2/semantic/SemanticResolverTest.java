...
class SemanticResolverTest {
    ...
    private static List<ExpressionNode> collect(ExpressionFileNode file) {
        ...
        file.assignments().forEach(assignment -> {
            switch (assignment) {
                case com.runestone.expeval2.ast.SimpleAssignmentNode simpleAssignment -> collect(simpleAssignment.value(), nodes);
                case com.runestone.expeval2.ast.DestructuringAssignmentNode destructuringAssignment -> collect(destructuringAssignment.value(), nodes);
            }
        });
    ...
    private static void collect(ExpressionNode node, List<ExpressionNode> nodes) {
        ...
        switch (node) {
            ...
            case com.runestone.expeval2.ast.UnaryOperationNode unaryOperationNode -> collect(unaryOperationNode.operand(), nodes);
            case com.runestone.expeval2.ast.PostfixOperationNode postfixOperationNode -> collect(postfixOperationNode.operand(), nodes);
            case com.runestone.expeval2.ast.ConditionalNode conditionalNode -> {
            ...
            case com.runestone.expeval2.ast.VectorLiteralNode vectorLiteralNode -> vectorLiteralNode.elements().forEach(element -> collect(element, nodes));
            case IdentifierNode ignored -> {
            }
            case com.runestone.expeval2.ast.LiteralNode ignoredLiteral -> {
            }
        }
    }
