...
public final class SpecificationGreaterOrEquals extends AbstractSpecificationFilterOperation {

    public SpecificationGreaterOrEquals(DataConversionService conversionService) {
        super(conversionService);
    }

    @Override
    public Class<? extends DefinedFilterOperation> operationType() {
        return GreaterOrEquals.class;
    }
    ...
    Predicate createPredicate(FilterData filterData, Root<?> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        ...
        return criteriaBuilder.greaterThanOrEqualTo(comparable(path), (Comparable) converted(filterData, path));
    }
}
