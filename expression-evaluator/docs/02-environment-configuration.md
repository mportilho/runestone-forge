...
- `overridable=false`: treated as a constant. The compiler folds it into the execution plan — no lookup at runtime, zero overhead. If you can override it in `compute()`, the override is silently ignored.
