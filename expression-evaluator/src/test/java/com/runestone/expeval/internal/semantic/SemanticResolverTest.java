...
class SemanticResolverTest {
    ...
    private static List<ExpressionNode> collect(ExpressionFileNode file) {
        ...
        file.assignments().forEach(assignment -> {
            switch (assignment) {
                case com.runestone.expeval.internal.ast.SimpleAssignmentNode simpleAssignment -> collect(simpleAssignment.value(), nodes);
                case com.runestone.expeval.internal.ast.DestructuringAssignmentNode destructuringAssignment -> collect(destructuringAssignment.value(), nodes);
            }
        });
    ...
    private static void collect(ExpressionNode node, List<ExpressionNode> nodes) {
        ...
        switch (node) {
            ...
            case com.runestone.expeval.internal.ast.UnaryOperationNode unaryOperationNode -> collect(unaryOperationNode.operand(), nodes);
            case com.runestone.expeval.internal.ast.PostfixOperationNode postfixOperationNode -> collect(postfixOperationNode.operand(), nodes);
            case com.runestone.expeval.internal.ast.ConditionalNode conditionalNode -> {
            ...
            case com.runestone.expeval.internal.ast.VectorLiteralNode vectorLiteralNode -> vectorLiteralNode.elements().forEach(element -> collect(element, nodes));
            case IdentifierNode ignored -> {
            }
            case com.runestone.expeval.internal.ast.LiteralNode ignoredLiteral -> {
            }
            case com.runestone.expeval.internal.ast.PropertyChainNode propertyChainNode ->
                propertyChainNode.chain().forEach(access -> {
                    if (access instanceof com.runestone.expeval.internal.ast.PropertyChainNode.MethodCallAccess m) {
                        m.arguments().forEach(arg -> collect(arg, nodes));
                    }
                });
            case com.runestone.expeval.internal.ast.TernaryOperationNode ternaryOperationNode -> {
