...
public final class AssignmentExpression {
     ...
     *
     * AssignmentExpression expr = AssignmentExpression.compile("x = a + b", environment, engine);
     * }</pre>
     *
     * @param source      expression source text; must not be blank
    ...
    public static AssignmentExpression compile(String source, ExpressionEnvironment environment, ExpressionEngine engine) {
        return Objects.requireNonNull(engine, "engine must not be null").compileAssignments(source, environment);
    }
